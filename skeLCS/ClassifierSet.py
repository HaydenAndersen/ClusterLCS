...
class ClassifierSet:
    ...
    def makeMatchSet(self,state_phenotype,exploreIter,elcs):
        ...
        #Covering
        while doCovering:
            newCl = Classifier(elcs,1,exploreIter,state,phenotype)
    ...
    def updateSets(self,elcs,exploreIter):
        ...
        for ref in self.matchSet:
            ...
            if ref in self.correctSet:
                ...
                self.popSet[ref].updateCorrectSetSize(elcs, correctSetNumerosity)
    ...
    def selectClassifierT(self,elcs):
        ...
        setList = [s for s in setList if self.popSet[s].matchCount > 10]
    ...
    def getPopTrack(self, accuracy, exploreIter, trackingFrequency, elcs):
        """ Returns a formated output string to be printed to the Learn Track output file. """
        trackString = str(exploreIter)+ "\t" + str(len(self.popSet)) + "\t" + str(self.microPopSize) + "\t" + str(accuracy) + "\t" + str(self.aveGenerality)  + "\t" + str(elcs.timer.returnGlobalTimer())+ "\n"
        if elcs.env.formatData.discretePhenotype: #discrete phenotype
            print(("Epoch: "+str(int(exploreIter/trackingFrequency))+"\t Iteration: " + str(exploreIter) + "\t MacroPop: " + str(len(self.popSet))+ "\t MicroPop: " + str(self.microPopSize) + "\t AccEstimate: " + str(accuracy) + "\t AveGen: " + str(self.aveGenerality)  + "\t Time: " + str(elcs.timer.returnGlobalTimer())))
        else: # continuous phenotype
            print(("Epoch: "+str(int(exploreIter/trackingFrequency))+"\t Iteration: " + str(exploreIter) + "\t MacroPop: " + str(len(self.popSet))+ "\t MicroPop: " + str(self.microPopSize) + "\t AccEstimate: " + str(accuracy) + "\t AveGen: " + str(self.aveGenerality) + "\t PhenRange: " +str(self.avePhenotypeRange) + "\t Time: " + str(elcs.timer.returnGlobalTimer())))
