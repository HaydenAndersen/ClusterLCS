...
class eLCS(BaseEstimator,ClassifierMixin, RegressorMixin):
    def __init__(self, learning_iterations=10000, track_accuracy_while_fit = False, N=1000, p_spec=0.5, discrete_attribute_limit=10,
                 ...
                 reboot_filename=None, verbose=0, movingAvgCount=50):
        ...
        if not isinstance(verbose, int) or verbose < -1:
            raise Exception("verbose param must be positive int or -1")

        if not isinstance(movingAvgCount, int) or movingAvgCount < -1:
            raise Exception("moving_avg_count param must be positive int or -1")
        '''
        Set params
        '''
        ...
        self.movingAvgCount = movingAvgCount
    ...
    def fit(self, X, y):
        ...
        if self.movingAvgCount == -1:
            self.movingAvgCount = len(y)
        ...
        while self.explorIter < self.learning_iterations:
            ...
            if self.verbose and (self.explorIter%self.verbose) == (self.verbose - 1) and self.explorIter > 0:
                self.population.runPopAveEval(self.explorIter, self)
                trackedAccuracy = accuracy #Accuracy over the last "trackingFrequency" number of iterations.
                self.population.getPopTrack(trackedAccuracy, self.explorIter+1,self.verbose, self) #Report learning progress to standard out and tracking file.
