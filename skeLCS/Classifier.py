...
class Classifier:
    ...
    def classifierCopy(self, toCopy, exploreIter):
        ...
        self.aveCorrectSetSize = copy.deepcopy(toCopy.aveCorrectSetSize)
    ...
    def updateCorrectSetSize(self, elcs, correctSetSize):
        """  Updates the average match set size. """
        if self.correctCount < 1.0 / elcs.beta:
            self.aveCorrectSetSize = (self.aveCorrectSetSize * (self.correctCount - 1) + correctSetSize) / float(
                self.correctCount)
        else:
            self.aveCorrectSetSize = self.aveCorrectSetSize + elcs.beta * (correctSetSize - self.aveCorrectSetSize)
    ...
    def getDelProp(self, elcs, meanFitness):
        ...
        if self.fitness / self.numerosity >= elcs.delta * meanFitness or self.matchCount < elcs.theta_del:
            deletionVote = self.aveCorrectSetSize * self.numerosity
        elif self.fitness == 0.0:
            deletionVote = self.aveCorrectSetSize * self.numerosity * meanFitness / (elcs.init_fit / self.numerosity)
        else:
            deletionVote = self.aveCorrectSetSize * self.numerosity * meanFitness / (self.fitness / self.numerosity)
        return deletionVote